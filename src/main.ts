...
import {
  exportCallGraphHtml,
  exportDataFlowGraphHtml,
} from "./output/visualizer";
import {
  transformPatternMatchVulnerabilitiesToSLMatches as transformMatches,
...
import {
  getAPIExported,
  reportAccessPaths,
  reportAPIExportedFunctions,
} from "./patternmatching/apiexported";
...
program
  ...
  .option(
    "-b, --basedir <directory>",
    "base directory for files to analyze (default: auto-detect)"
  )
  ...
  .option(
    "-n, --graal-home <directory>",
    "home of graal-nodejs (default: $GRAAL_HOME)"
  )
  .option(
    "-d, --dynamic <file>",
    "generate call graph dynamically, no static analysis"
  )
  .option("--approx", "enable approximate interpretation")
  .option(
    "--approx-only <file>",
    "perform approximate interpretation, no static analysis"
  )
  .option(
    "--approx-load <file>",
    "use pre-computed approximate interpretation results"
  )
  .option(
    "--approx-store <file>",
    "store approximate interpretation results (use with --approx)"
  )
  .option(
    "-p, --patterns <file...>",
    "files containing API usage patterns to detect"
  )
  ...
  .option(
    ...
    "include only dependencies in this list"
  )
  .option(
    "--exclude-packages <package...>",
    "exclude dependencies in this list"
  )
  ...
  .option(
    "--tokens-json <file>",
    "save tokens for constraint variables as JSON file"
  )
  ...
  .option(
    "--gc",
    "enable garbage collection for more accurate memory usage reporting"
  )
  .option("--typescript", "enable TypeScript type inference (use with -p)")
  .option(
    "--api-usage",
    "report API usage of external packages (implies --ignore-dependencies)"
  )
  .option("--api-exported", "report API of modules")
  .option(
    "--find-access-paths <location>",
    "find access paths for source location (file:line)"
  )
  .option("--higher-order-functions", "report higher-order functions")
  .option(
    "--zeros",
    "report calls with zero callees and functions with zero callers"
  )
  .option(
    "--exclude-entries <glob...>",
    "files to exclude when specifying entry directories"
  )
  .option(
    "--tracked-modules <glob...>",
    "modules to track usage of (default: auto-detect)"
  )
  ...
  .option(
    "--no-callgraph-external",
    "omit heuristic external callbacks in call graph"
  )
  ...
  .option(
    "--max-indirections <number>",
    "limit number of function call and property write indirections"
  )
  .option(
    "--full-indirection-bounding",
    "enable indirection bounding for method calls and property reads (use with --max-indirections)"
  )
  .option(
    "--typescript-library-usage <file>",
    "save TypeScript library usage in JSON file, no analysis"
  )
  .option(
    "--modules-only",
    "report reachable packages and modules only, no analysis"
  )
  .option(
    "--compare-callgraphs",
    "compare two call graphs given as JSON files, no analysis"
  )
  .option(
    "--reachability",
    "compare call graph reachability (use with -s or --compare-callgraphs)"
  )
  .option(
    "--library",
    "assume program is a library (default: true if in node_modules)"
  )
  ...
  .option(
    "--patch-dynamics",
    "enable dynamic property access patching heuristic"
  )
  ...
  .option(
    "--obj-spread",
    "enable model of spread syntax for object literals ({...obj})"
  )
  .option(
    "--native-overwrites",
    "allow overwriting of native object properties"
  )
  .option("--ignore-imprecise-native-calls", "ignore imprecise native calls")
  .option("--matches-file <file>", "save matches in JSON file")
  .option(
    "--reachable-packages-file <file>",
    "saves reachable packages in JSON file"
  )
  .usage("[options] [files]")
  .addHelpText(
    "after",
    "\nAll modules reachable by require/import from the given files are included in the analysis\n" +
      ...
      `Memory limit is ${getMemoryLimit()}MB.${
        PKG
          ? ""
          : " Change with, for example: NODE_OPTIONS=--max-old-space-size=8192"
      }`
  )
  .action(main)
  .showHelpAfterError()
...
async function main() {
  ...
  if (options.logfile) logToFile(options.logfile);
  ...
  if (PKG)
    for (const opt of ["dynamic"] as const)
      if (options[opt]) {
        logger.error(
          `Error: Option --${opt} not available in binary executable`
        );
        process.exitCode = -1;
        return;
      }
  ...
  if (options.patterns && options.vulnerabilities) {
    // TODO: also check this in server.ts
    logger.error(
      "Error: Options --patterns and --vulnerabilities cannot be used together"
    ); // pattern match confidence computation requires relevant libraries to be external
    process.exitCode = -1;
    return;
  }
  ...
  if (options.gc && typeof gc !== "function") {
    ...
    if (t.status === null) {
      ...
      process.exitCode = -1;
    } else process.exitCode = t.status;
    return;
  }
  ...
  if (options.dynamic) {
    ...
    if (options.testGraal) {
      ...
      const t = spawnSync(node, [
        "-e",
        "process.exit(typeof Graal === 'object' ? 0 : -1)",
      ]);
      if (t.status === null) {
      ...
      if (t.status !== 0) {
        logger.error(
          "Error: 'node' is not graal-nodejs, try option --graal-home or environment variable GRAAL_HOME"
        );
        process.exitCode = -1;
        return;
      }
    ...
    for (const f of readdirSync(dir, { withFileTypes: true }))
      if (f.isFile()) {
        ...
        if (p.startsWith(`${dyn}-`)) {
          // instrumented execution has produced $JELLY-OUT-<PID> files
          cgs.push(JSON.parse(readFileSync(p, "utf-8")) as CallGraph);
          unlinkSync(p);
        }
      }
    logger.verbose(
      `Collected ${cgs.length} call graphs from instrumented execution`
    );
    ...
    let files;
    try {
      if (!autoDetectBaseDir(program.args)) return;
      files = expand(program.args);
    } catch (e) {
      logger.info(
        `Error: ${
          e instanceof Error
            ? "code" in e && e.code === "ENOENT" && "path" in e
              ? `File not found ${e.path}`
              : e.message
            : "Unable to expand paths"
        }`
      );
      return;
    }
    if (logger.isVerboseEnabled()) {
      ...
      for (const file of files) logger.verbose(`  ${file}`);
    }
    ...
    if (options.approxOnly) {
      ...
      try {
        ...
        if (options.diagnostics) p.printDiagnostics();
      ...
      closeSync(fd);
      logger.info(
        `TypeScript library usage written to ${options.typescriptLibraryUsage}`
      );
    } else {
      let tapirPatterns, patterns, globs, props, vulnerabilityDetector;
      if (options.patterns) {
        tapirPatterns = removeObsoletePatterns(
          loadTapirDetectionPatternFiles(options.patterns)
        );
      ...
      if (options.vulnerabilities || options.vulnerabilitiesJSON) {
        logger.info(
          `Loading vulnerability patterns from ${options.vulnerabilities}`
        );
        vulnerabilityDetector = new VulnerabilityDetector(
          JSON.parse(
            options.vulnerabilities
              ? readFileSync(options.vulnerabilities, "utf8")
              : options.vulnerabilitiesJSON!
          ) as Array<Vulnerability>
        ); // TODO: use when setting globs and props? (see also server.ts)
      ...
      if (options.callgraph) out.reportCallGraph();

      if (options.tokens) out.reportTokens();

      if (options.tokensJson) out.saveTokens(options.tokensJson);
      ...
      if (options.modulesOnly) out.reportReachablePackagesAndModules();

      if (options.soundness)
        compareCallGraphs(
          options.soundness,
          "<computed>",
          out.callGraphToJSON(files),
          false,
          options.reachability
        );
      ...
      if (options.apiExported || options.findAccessPaths) {
        ...
        if (options.apiExported) reportAPIExportedFunctions(r);
        if (options.findAccessPaths)
          reportAccessPaths(f, r, options.findAccessPaths);
      }

      if (options.higherOrderFunctions) out.reportHigherOrderFunctions();
      ...
      if (options.variableKinds) out.reportVariableKinds();

      if (options.reachablePackagesFile)
        writeFileSync(
          options.reachablePackagesFile,
          JSON.stringify(getReachablePackages(solver), null, 2)
        );
    }
  }
}
